<x-splade-modal>
    ...
    <x-splade-toggle :data="false">
        ...
        <table class="hover:table-fixed">
            <tbody>
                <tr>
                    ...
                    @if ($ret['file'] != '')
                        <td>
                            <a href="{{ route('document-files.download', $ret['id']) }}" donwload title="Download Zip">
        ...
        <div v-show="toggled">
            <x-splade-form  method="post" :action="route('document-files.upload',['id' => $ret['id']])" class="mt-4 space-y-4" preserve-scroll>
    ...
    <table class="hover:table-fixed">
        <tbody>
            @for ($i = 0; $i<count($ret['files']); $i++)
                <tr>
                    <td style="width:25px;">
                        <a href="{{ route('document-files.delete', ['id' => $ret['id'],'nomearq' => $ret['files'][$i][0]]) }}" title="Excluir Documento">
                    ...
                    <td>
                        <a href="{{ route('document-files.openfile', $ret['files'][$i][0] ) }}" title="Abrir Documento" target="__blank">
                            {{ $ret['files'][$i][0] }}
                        </a>
                    </td>
                    {{-- <td>
                        ...
                        @if(substr($ret['files'][$i][0], strrpos($ret['files'][$i][0], '.') + 1) == 'pdf')
                            <a href="{{ url('/') . '/uploads/downloads/' . auth('sanctum')->user()->id . '/' . $ret['files'][$i][0] }}" onclick="window.open(this.ref, 'new', 'popup'); return false;">{{ $ret['files'][$i][0] }}</a>
                        @else
